#!/usr/bin/env python3
"""Generate a static HTML manuals site from markdown source and dashboard markdown outputs."""
...
SITE_DIR = ROOT / "site"
DASHBOARD_DOCS_DIR = SITE_DIR / "dashboard"
OUTPUT_DIR = SITE_DIR / "manuals"
...
def source_rel_for(path: Path) -> Path:
    try:
        return path.relative_to(DOCS_SOURCE_DIR)
    except ValueError:
        pass
    try:
        return Path("dashboard") / path.relative_to(DASHBOARD_DOCS_DIR)
    except ValueError as exc:
        raise ValueError(f"Unsupported markdown source path: {path}") from exc


def collect_markdown_files() -> list[Path]:
    files: list[Path] = []
    for base_dir in (DOCS_SOURCE_DIR, DASHBOARD_DOCS_DIR):
...
def rewrite_markdown_links(text: str, current_source: Path, current_output: Path) -> str:
    ...
    def replace(match: re.Match[str]) -> str:
        ...
        if target_path.suffix.lower() == ".md":
            ...
            try:
                resolved_rel = source_rel_for(resolved)
            except ValueError:
                return match.group(0)
...
def render_page(page: Page, body_html: str, toc_html: str, prev_page: Page | None, next_page: Page | None) -> str:
    ...
    home_href = os.path.relpath(SITE_DIR / "index.html", page_path.parent).replace(os.sep, "/")
    dashboard_href = os.path.relpath(SITE_DIR / "dashboard" / "index.html", page_path.parent).replace(os.sep, "/")
    details_href = os.path.relpath(SITE_DIR / "dashboard" / "details.html", page_path.parent).replace(os.sep, "/")
    index_href = os.path.relpath(OUTPUT_DIR / "index.html", page_path.parent).replace(os.sep, "/")
    costs_href = os.path.relpath(SITE_DIR / "costs" / "index.html", page_path.parent).replace(os.sep, "/")
...
<body>
  <div class="page">
    ...
    <section class="hero">
      <h1>小黑托昵文档中心</h1>
      <p>这里收的是 `docs/source/` 手册和 `site/dashboard/` 输出摘要对应的网页阅读版文档。老板可以直接读策略和经营手册，员工也可以用手机打开培训、接待、检查清单，不需要再看原始 Markdown。</p>
      <div class="hero-actions">
      ...
      <div class="meta-row">
        ...
        <span class="meta-chip">文档范围：docs/source/*.md 与 site/dashboard/*.md</span>
      </div>
    </section>
...
def build_pages() -> list[Page]:
    ...
    for source_path in source_files:
        rel = source_rel_for(source_path)
